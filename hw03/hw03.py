...
def g_iter(n):
    ...
    while i <= n:
        if i <= 3:
            ...
            listA = listA + [listA[i-1] + 2*listA[i-2] + 3*listA[i-3]]
        i+=1
...
def missing_digits(n):
    ...
    #将一个数字分为两部分，一部分是个位另一部分是其他部分，一个一位数和一个多位数，然后检查多位数和一位数中间是否相差数字，然后递归
    #因为中间相差数字只统计个数不要求具体数字所以做差即可
    ...
    def helper(m):
        if m <= 10:
            ...
            m_last = m % 10
            m_without_last = m // 10
            nums = m_last - m_without_last%10
            if nums != 0:nums-=1
            return helper(m_without_last) + nums
...
def count_change(total):
    ...
    return helper(total,biggest_coin(total))
